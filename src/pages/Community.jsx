...
const getInitials = (p) =>
  getDisplayName(p).split(" ").slice(0, 2).map((s) => s[0]?.toUpperCase()).join("");
...
// AVATAR
// =========================================================
function Avatar({ person, size = "md", online = false, ring = false }) {
  const sizes = {
    ...
    <div className="relative shrink-0">
      {person?.avatar_url ? (
        <img
          ...
          className={`${sizes[size]} rounded-full object-cover border border-gray-700 ${ring ? "ring-2 ring-yellow-400/40" : ""}`}
        />
      ) : (
        <div className={`${sizes[size]} rounded-full bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 flex items-center justify-center font-bold text-gray-300 ${ring ? "ring-2 ring-yellow-400/40" : ""}`}>
          {getInitials(person)}
        </div>
      )}
      {online && (
